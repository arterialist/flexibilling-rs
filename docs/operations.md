...
Leave a record pending after an unexpected error so a later worker can retry.

## Product grants

`fund_customer` accepts external product IDs and a payment reference. It checks
for an existing ledger transaction before applying each active mapping.

- `TopUp` adds to the current balance;
- `MonthlyQuota` replaces it with the configured grant.
...
- Do not put secrets in `event_metadata` or cache activity events.
